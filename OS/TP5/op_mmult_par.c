...
int try_lock (pthread_mutex_t *lock ){
  if (*lock == go){
    *lock = keep_out;
  }
  return *lock;
...
static void compute_coeff( mat m1, mat m2, mat res, size_t r, size_t c, size_t dim ) {
   ...
   */
for (k = 0; k < dim; k++){
  if (pthread_create(tid, NULL, &thread_main, thr_arg) != 0) { //1 = identifiant du thread; 2 = NULL; 3 = point d'entree du thread; 4 = pointeur sur parametres du thread
    printf("Erreur de creation de threat\n");
    //up_to_success = pthread_join(tid, );
    exit(EXIT_FAILURE);
  }
}
